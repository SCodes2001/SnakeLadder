...
async function choicePlayer(noOfPlayer) {
    ...
    red.addEventListener('click', function run() {   
        ...
        if(count==noOfPlayer){
            ...
            setTimeout(()=>{
                blinks.innerHTML="";
                chooseColour.style.display = "none";},1000);
                setTimeout(()=>{
      ...
      dice.addEventListener('click',async function Play(){
        ...
        setTimeout(()=>{
            if(redtokenpos==100){
                ...
                setTimeout(()=>{
                    winning.innerHTML="RED WINS THE GAME";
     ...
     dice.addEventListener('click',async function Play(){
        ...
        setTimeout(()=>{
            if(bluetokenpos==100){
                ...
                winning.style.visibility="visible";
                setTimeout(()=>{
                    winning.innerHTML="BLUE WINS THE GAME";
                    winning.style.color="black";
                    winning.style.backgroundColor="blue"
     ...
     dice.addEventListener('click',async function Play(){
        ...
        setTimeout(()=>{
            if(yellowtokenpos==100){
                ...
                winning.style.visibility="visible";
                setTimeout(()=>{
                    winning.innerHTML="YELLOW WINS THE GAME";
                    winning.style.color="black";
...
async function Play(a,arr){
    ...
    setTimeout(()=>{
        dice.innerHTML=`${score}`
        let pos1=redtoken.offsetLeft;
        let pos2=score*7;
        let final=(pos1+pos2);
        redtoken.style.left=`${final}vmin`;
        a=a+1;
        dice.removeEventListener("click",Play);     
    },700);
    dice.removeEventListener("click",()=>Play); 
